...
For a reviewed contact with a saved raw page or API receipt, include both source_receipt and source_sha256. The receipt path is relative to the campaign config and must stay within the config directory’s parent; source_sha256 is the lowercase SHA-256 of the receipt bytes. Preparation verifies the file and hash before writing a review, and rejects receipts containing credential-like values. Omit both fields only for older contact inputs that have not yet been migrated to receipt-backed provenance.
